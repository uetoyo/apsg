...
"""
This module contains the classes representing the structural features.
We recognise two main categories:
1. _linear_ features
2. _planar_ features
"""
...
from apsg._helpers import cosd, sind

__all__ = ("Lin", "Fol", "Pair", "Fault", "Group", "PairSet", "FaultSet")


class Feature(ABC):
    """
    The marker abstract base for geological features.
    """


class LinearFeature(Feature):
    """
    The linear structural feature.
    """


class PlanarFeature(Feature):
    """
    The planar structural feature.
    """


class Lineation(LinearFeature):
    """
    previously `Lin`
    """


class Foliation(PlanarFeature):
    """
    previously `Fol`
    """
...
# #############################################################################
# These classes are imported from core.py module and will be refactored
# #############################################################################
...
def G(s, type=Lin, name="Default") -> Group:
    """
    Create a group from space separated string of azimuths and inclinations.

    @todo refactor as class method of `Group` class.
    """

    vals = np.fromstring(s, sep=" ")

    return Group.from_array(vals[::2], vals[1::2], type=type, name=name)
